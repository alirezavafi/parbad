...
using Persian.Plus.PaymentGateway.Gateways.VirtualGateway;

namespace Persian.Plus.PaymentGateway.VirtualGateway
{
    public static class ParbadVirtualGatewayMiddlewareExtensions
    {
        /// <summary>
        /// Adds the Persian.Plus.PaymentGateway.Core Virtual Gateway middleware to the pipeline if the current
...
#else
            var hostEnvironment = builder.ApplicationServices.GetRequiredService<Microsoft.AspNetCore.Hosting.IHostingEnvironment>();
            var isDevelopment = Microsoft.AspNetCore.Hosting.HostingEnvironmentExtensions.IsDevelopment(hostEnvironment);
#endif
            return builder.UseParbadVirtualGatewayWhen(context => isDevelopment);
        }

        /// <summary>
        /// Adds the Persian.Plus.PaymentGateway.Core Virtual Gateway middleware to the pipeline.
        /// </summary>
        /// <param name="builder"></param>
        ...
        public static IApplicationBuilder UseParbadVirtualGateway(this IApplicationBuilder builder)
        {
            ...
            {
                throw new InvalidOperationException("Cannot get Persian.Plus.PaymentGateway.Core Virtual gateway path value. " +
